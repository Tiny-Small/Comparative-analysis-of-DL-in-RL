...
import traceback
import sys
from agents.util import nearestPoint, TimeoutFunction, TimeoutFunctionException
...
class Grid:
    ...
    def __str__(self):
        ...
        for y in range(self.height):
            ...
            out.append("".join(["T" if self[x][y] else "F" for x in range(self.width)]))
        out.reverse()
...
class Game:
    ...
    def __init__(self, agents, display, rules, startingIndex=0, muteAgents=False, catchExceptions=False):
        ...
        self.stepLimit = STEPLIMIT  # New attribute for step limit
        self.stepsTaken = 0  # New attribute to count steps
    ...
    def run(self):
        ...
        while not self.gameOver:
            ...
            # Increment step count if Pac-Man (agentIndex == 0) moved
            if agentIndex == 0:
                self.stepsTaken += 1
            # Check if the step limit has been reached
            if self.stepLimit is not None and self.stepsTaken >= self.stepLimit:
                print(f"Step limit of {self.stepLimit} reached. Ending game.")
                self.gameOver = True
                break
