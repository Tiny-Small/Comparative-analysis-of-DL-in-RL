...
class DQN(tf.keras.Model):
    def __init__(self, params):
        ...
        self.embedding_layer = ChannelEmbeddingLayer(self.NUM_CHANNELS, 16)
...
class ChannelEmbeddingLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        embeddings_expanded = tf.reshape(embeddings, (1, 1, 1, 1, inputs.shape[-1], -1))
        ...
        output = tf.reshape(output, (tf.shape(output)[0], -1, tf.shape(output)[-1]))
