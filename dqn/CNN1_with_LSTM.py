...
class DQN(tf.keras.Model):
    def __init__(self, params):
        ...
        # Total params: 229524 (896.58 KB)
        # Trainable params: 229524 (896.58 KB)
        # Non-trainable params: 0 (0.00 Byte)

        self.conv1 = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), padding='same')
        self.ln1 = tf.keras.layers.LayerNormalization()

        self.lstm1 = tf.keras.layers.LSTM(186)
    ...
    def call(self, inputs, training=False):
        ...
        timesteps = x1.shape[3]
        features = x1.shape[1] * x1.shape[2]
        x = tf.keras.layers.Reshape((timesteps, features))(x1)
