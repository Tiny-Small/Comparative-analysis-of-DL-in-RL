...
canvas_tfonts = ["times new roman", "lucida console"] if _Windows else ["times", "lucidasans-24"]
...
def circle(pos, r, outlineColor, fillColor, endpoints=None, style="pieslice", width=2):
    ...
    return _canvas.create_arc(
        x0, y0, x1, y1, outline=outlineColor, fill=fillColor, extent=e[1] - e[0], start=e[0], style=style, width=width
    )
...
def remove_from_screen(x, d_w=tkinter._tkinter.DONT_WAIT):
    global _root_window
    _canvas.delete(x)
    _root_window.dooneevent(d_w)
...
def move_to(object, x, y=None, d_w=tkinter._tkinter.DONT_WAIT):
    if y is None:
        try:
            x, y = x
        except:
...
def move_by(object, x, y=None, d_w=tkinter._tkinter.DONT_WAIT, lift=False):
    if y is None:
        try:
            x, y = x
        except:
